...
func GetSessionPhaseTime(w http.ResponseWriter, r *http.Request) {
    sessionID := r.URL.Query().Get("session_id")
    phase := r.URL.Query().Get("phase")
    
    if sessionID == "" || phase == "" {
        w.WriteHeader(http.StatusBadRequest)
        json.NewEncoder(w).Encode(map[string]string{"error": "session_id and phase are required"})
        return
    }

    // Get session phase end time from database
    var endTime time.Time
    err := database.GetDB().QueryRow(`
        SELECT end_time FROM session_phases 
        WHERE session_id = ? AND phase = ? 
        ORDER BY created_at DESC LIMIT 1`,
        sessionID, phase).Scan(&endTime)

    if err != nil {
        if err == sql.ErrNoRows {
            // If no phase tracking exists, calculate based on session start time
            var startTime time.Time
            var agendaJSON []byte
            err := database.GetDB().QueryRow(`
                SELECT start_time, agenda FROM gd_sessions WHERE id = ?`,
                sessionID).Scan(&startTime, &agendaJSON)
            ...
            // Parse agenda to get phase duration
            var agenda struct {
                PrepTime   int `json:"prep_time"`
                Discussion int `json:"discussion"`
                Survey     int `json:"survey"`
            }
            json.Unmarshal(agendaJSON, &agenda)
            
            var phaseDuration int
            switch phase {
            case "prep":
                phaseDuration = agenda.PrepTime
            case "discussion":
                phaseDuration = agenda.Discussion
            case "survey":
                phaseDuration = agenda.Survey
            default:
                phaseDuration = 0
            }

            remaining := int(time.Until(startTime.Add(time.Duration(phaseDuration)*time.Minute)).Seconds())
            if remaining < 0 {
                remaining = 0
            }
...
func UpdateSessionPhaseTime(w http.ResponseWriter, r *http.Request) {
    var req struct {
        SessionID       string `json:"session_id"`
    ...
    endTime := time.Now().Add(time.Duration(req.RemainingSeconds) * time.Second)

    // Update or insert phase tracking
    _, err := database.GetDB().Exec(`
        INSERT INTO session_phase_tracking (session_id, phase, end_time)
        VALUES (?, ?, ?)
        ON DUPLICATE KEY UPDATE end_time = VALUES(end_time)`,
        req.SessionID, req.Phase, endTime)

    if err != nil {
        w.WriteHeader(http.StatusInternalServerError)
        json.NewEncoder(w).Encode(map[string]string{"error": "Failed to update phase time"})
        return
    }
...
func CompleteSessionPhase(w http.ResponseWriter, r *http.Request) {
    ...
    // Mark phase as completed by setting end time to now
    _, err := database.GetDB().Exec(`
        UPDATE session_phase_tracking 
        SET end_time = NOW(), completed = TRUE
        WHERE session_id = ? AND phase = ?`,
        req.SessionID, req.Phase)

    if err != nil {
        w.WriteHeader(http.StatusInternalServerError)
        json.NewEncoder(w).Encode(map[string]string{"error": "Failed to complete phase"})
        return
    }
